;(function(name, definition) {
   ...
   * @returns {Promise<any>}
   */
  function fileToBase64String(file, format = ['jpg', 'jpeg', 'png', 'gif'], size = 20 * 1024 * 1024, formatMsg = '文件格式不正确', sizeMsg = '文件大小超出限制') {
   ...
   * 递归生成树形结构
   */
  function getTreeData(data, pid, pidName = 'parentId', idName = 'id', childrenName = 'children', key) {
    let arr = []
   ...
   * 追溯父节点
   */
  function traceParentNode(pid, data, rootPid, pidName = 'parentId', idName = 'id', childrenName = 'children') {
    let arr = []
   ...
   */
  const createTree = (items, id = null, link = 'pid') => {
  ...
  const sumNum = (...arr) => [...arr].reduce((acc, val) => acc + val, 0)
